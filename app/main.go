...
func main() {
	...
	http.HandleFunc("/Vaccines/List", models.GetVaccines)
	err := http.ListenAndServe(fmt.Sprintf(":%d", port), nil)
	if err != nil {
		log.Fatal(err)
	}
	database, err := sql.Open("mysql", "root:MYSQLaccount123@tcp(127.0.0.1:3306)/aartas")
	if err != nil {
		log.Fatal(err)
	}
	defer database.Close()
	fmt.Println("Successfully connected to the database!")
	vaccinestable, err := database.Query("SELECT id,name,vaccine_category_id FROM vaccines")
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, vaccines := range vaccine {
		fmt.Printf("Id: %d, Vaccine_name: %s, Vaccine_category_id: %d\n", vaccines.Id, vaccines.Vaccine_name, vaccines.Vaccine_category_id)
	}
